...
function isPrivateWindow(callback) {
	PrivateWindow.then(function(is_private1) {
		console.log(is_private1);
		callback(is_private1);
	});
}
	isPrivateWindow(function(is_private1) {
		document.getElementById('incog').value = is_private1 === null ? 'cannot detect' : is_private1 ? 'Yes' : 'No';
	});
...
function isPrivateWindow(callback) {
	...
	isPrivateWindow(function(is_private) {
		document.getElementById('incog_status').innerHTML = is_private === null ? 'cannot detect' : is_private ? "<img src='../images/incognito.png' />" : "<img src='../images/non_incognito.png' />";
	});